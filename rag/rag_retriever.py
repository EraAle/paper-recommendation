...
class RAGRetriever:
    def __init__(self, model_name:str="all-MiniLM-L6-v2", use_cuda:bool=True):
        ...
        logging.info(f"Initializing RAGRetriever with model: {model_name} on {self.device}")
        self.encoder = model.get_model(model_name).to("cuda") if use_cuda else model.get_model(model_name)
        logging.info(f"Success: Initializing RAGRetriever with model: {model_name} on {self.device}")

    def get_embedding(self, input_str: str):
        """
        Encode a single string into an embedding tensor.
        """
        logging.debug(f"Encoding string: {input_str[:30]}...")
    ...
    def get_embeddings(self, input_list: list[str]):
        ...
        """
        logging.debug(f"Encoding list of {len(input_list)} strings")
    ...
    def run(self, query: str, refs: list, top_k: int = 3):
        ...
        """
        logging.info(f"Running retrieval for query: {query}")
        ...
        logging.info(f"Top-{top_k} results ready")
        return top_indices